            ...
            var results = from b in DBContext.Bookings
                ...
                select
                    new GroupedBooking
                    {
                        ...
                        TotalAmount = (decimal) (gr.Where(q => q.TicketId == gr.Key.TicketId).Select(p => p.Price * gr.Key.NumberOfTickets).FirstOrDefault() == null ? (decimal)0.00 : gr.Where(q => q.TicketId == gr.Key.TicketId).Select(p => p.Price * gr.Key.NumberOfTickets).FirstOrDefault()),
